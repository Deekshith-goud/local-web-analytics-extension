import { engine, classifier } from "./background/engine-instance";
...
import { invalidateTodayCache, maintenanceEventsLog } from "./background/cache-service";
...
async function runMaintenance(): Promise<void> {
  ...
  try {
    ...
    logger.info(`[Background] Compaction completed successfully: Pruned ${result.rowsDeleted} rows.`);
  } catch (err) {
    maintenanceInProgress = false;
